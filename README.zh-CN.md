...
- 设置中心：配置平台名称、界面语言、主题、信息密度、AI 模型、默认环境、通知策略和 Agent 启动策略。
- 账号权限：支持用户名/手机号密码登录、个人信息页、反馈入口和 viewer/operator/admin 角色边界。
...
- **账号权限**：用户名/手机号密码登录、个人信息、反馈和角色权限边界。

辅助管理能力：

- **Dashboard**：平台概览和关键入口。
- **设置页**：平台名称、界面语言、主题、信息密度、AI 模型、默认环境、通知策略和 Agent 启动策略。
