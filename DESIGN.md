...
- 第一阶段支持 `zh-CN` 和 `en`。
- 文案抽离到 typed content 模块，例如 `content/navigation.ts`、`content/settings.ts`、`content/ai.ts`。
- 导航、页面标题、按钮、空状态、表单提示、AI 快捷模板和错误信息都应从 locale 内容读取。
