...
export default async function Dashboard() {
  ...
  const { data: recentTasks } = supabase
    ? await supabase
      .from('tasks').select('id, display_id, parameters, status, environment, created_at, projects(name), test_suites(name, suite_key)')
              ...
              {(recentTasks as RecentTaskRow[]).map((task) => {
                ...
                return (
                  <tr key={task.id} className="transition-colors" style={{ borderBottom: '1px solid var(--border)' }}>
                    ...
                    <td className="px-5 py-3" style={{ color: 'var(--text-secondary)' }}>{testScopeDisplayName(task.test_suites, copy.common.testScopes) || '-'}</td>
                ...
                <Link key={task.id} href={`/tasks/${ref}`} className="rounded-xl p-4" style={{ background: 'var(--surface-soft)', border: '1px solid var(--border)' }}>
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <div className="mt-1 text-xs truncate" style={{ color: 'var(--text-muted)' }}>{testScopeDisplayName(task.test_suites, copy.common.testScopes) || '-'}</div>
                    </div>
